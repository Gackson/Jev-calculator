...
**Notes · 全局 prompt** 可以写鼓励的话或任何想尝试的 prompt，每次判断都会带上，对所有模式生效；不用时可以折叠。
...
- 每一步模型请求都包含同一份 `state.notes`，包括符号、数字、相等 / 大小、字符、像素 / 落笔 / 移动 / 抬笔 / 结束判断。关闭“带入之前的预测结果”仅去掉 Choice 的历史预测，Notes 仍然生效。
